...
DISCORD_TOKEN = "MTQ0NjM3MjQzMjI0NzI2MzI2Mg.GKd7L9.rRx9jABgB_vpZcmHm1kuit16XVvcLI9V0RUpKw" 
...
def check_trello_sync(username):
    ...
    try:
        ...
        if not roblox_id:
            return "❌ User ID not resolvable for friend check."
        ...
        output += f"**--- ⚠️ TRELLO BLACKLIST RESULTS ---**\n"
        
        if flagged:
            output += "🚨 **Flagged Friends Found!**\n"
            for name, list_type, fid in flagged:
                output += f" • `{name}` - **{list_type}** - ROBLOX ID: {fid}\n"
        else:
            output += "✅ No flagged friends found on Trello list."
        return output
    except Exception as e:
        return f"❌ Failed Step 6 (Trello check): An internal error occurred: `{e}`"
...
async def roblox_check(interaction: discord.Interaction, username: str):
    ...
    await interaction.followup.send(f"🔎 Starting full analysis for **{username}**... This may take a moment.")
    
    async with aiohttp.ClientSession() as session:
        try:
            # --- STEP 1 & 2: Get ID, Stats, and History ---
            await interaction.followup.send("➡️ **Step 1/6 & 2/6**: Fetching ID, Account Stats, and Prior Usernames...")
            ...
            stats_output = (
                ...
                f"✅ **Followers**: {followers} | **Friends**: {friends}\n"
                f"✅ **History**: {prior_names_str}"
            )
            await interaction.followup.send(stats_output)

            # --- STEP 3 & 4: Badges and Dates ---
            await interaction.followup.send("➡️ **Step 3/6 & 4/6**: Fetching Badges and Award Dates...")
            badges = await fetch_badges(session, user_id)
            dates, highlights = await fetch_award_dates(session, user_id, badges)
            await interaction.followup.send(f"✅ Found **{len(badges)}** badges. Generating graph...")

            # --- PLOTTING (CPU-Bound, run in executor) ---
            if dates:
                await interaction.followup.send("📈 **Plotting Badge Graph** (Running in background thread)...")
                ...
                await interaction.followup.send("⚠️ Not enough badge data to plot graph.")

            # --- STEP 5: Group Info ---
            await interaction.followup.send("➡️ **Step 5/6**: Fetching Group Memberships...")
            irf_groups, enemy_groups = await fetch_groups(session, user_id)

            group_output = "**--- 👥 GROUP MEMBERSHIPS ---**\n"
            group_output += "**IRF Groups:**\n"
            group_output += "\n".join([f" • {g}" for g in irf_groups]) if irf_groups else " • None\n"
            group_output += "\n**Enemy Groups:**\n"
            group_output += "\n".join([f" • {g}" for g in enemy_groups]) if enemy_groups else " • None\n"
            await interaction.followup.send(group_output)
            
            # --- STEP 6: Trello Blacklist Check (Synchronous, run in executor) ---
            await interaction.followup.send("➡️ **Step 6/6**: Trello Blacklist Check (Friends)...")
            ...
            await interaction.followup.send(f"🏁 **Analysis for {username} Complete!**")

        except ValueError as e:
            await interaction.followup.send(f"❌ **Error:** {e}")
        except Exception as e:
            await interaction.followup.send(f"❌ An unexpected error occurred: `{e}`. Check the console for details.")
